...
function UploadEncrypt() {
  ...
  const [uploads, setUploads] = useState<Array<{ uploadLink: string; dynamicLink: string }>>([]);
  ...
  const handleEncryptUpload = async () => {
    ...
    try {
      ...
      setUploads((prevUploads) => [
        ...prevUploads,
        {
          uploadLink: uploadResult.protocolLink,
          dynamicLink: uploadResult.dynamicLinks[0],
        },
      ]);
    } catch (err) {
      alert(err);
                  ...
                  <table className="mt-4">
                    ...
                    <tbody>
                      {uploads.map((upload, index) => (
                        <tr key={index}>
                          <td>
                            <a href={upload.uploadLink} target="__blank">
                              {upload.uploadLink}
                            </a>
                          </td>
                          <td>
                            <a href={`https://${upload.dynamicLink}`} target="__blank">
                              {upload.dynamicLink}
                            </a>
                          </td>
                        </tr>
